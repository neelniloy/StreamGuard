...
async function startServer() {
  ...
  app.get("/api/proxy/plaintext", async (req, res) => {
    ...
    try {
      // Use an IPTV / Player User-Agent that Xtream Codes providers expect and allow
  ...
  app.get("/api/proxy/json", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        // If it isn't valid JSON, still send the text response safely to be handled client-side
        res.setHeader("Access-Control-Allow-Origin", "*");
        res.send(text);
      }
    } catch (error: any) {
  ...
  app.get("/api/stream", async (req, res) => {
    ...
    try {
      ...
      // Read content type or default to MPEG-TS container
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`IPTV Player Proxy Server running on port ${PORT}`);
  });
}
